...
public class SignupServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       response.setContentType("text/html;charset=UTF-8");
        ...
        String xUser,xPass,xRePass;
        xUser = request.getParameter("user");
        xPass = request.getParameter("pass");
        xRePass = request.getParameter("repass");
        UserDAO udao = new UserDAO();
        User x = udao.getUser(xUser);
        if(!xRePass.equals(xPass)){
            request.setAttribute("mes", "Repeat Password is incorrect.");            
            request.getRequestDispatcher("Login.jsp").forward(request, response);
        } else {
            if(x==null){
                x=udao.addUser(xUser,xPass);
